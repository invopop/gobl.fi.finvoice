...
- **Due date**: payment terms must include exactly one dated entry
  (`EpiDateOptionDate`); Finvoice has no instalments.
...
- **Texts are fitted, identifiers are not.** The addon refuses identifiers
  too long for their element before conversion. Long names and texts are
  spread over the repeats the schema allows and cut beyond them. An email or
  web address too long for its element, and a postal address without a town
  and post code, are left out.
